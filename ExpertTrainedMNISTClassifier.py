...
class HierarchicalNN(nn.Module):
    def __init__(self):
        ...
        self.odd_expert = nn.Linear(128, 5)
        self.even_expert = nn.Linear(128, 5)
    ...
    def forward(self, x):
        ...
        even_logits = self.even_expert(features)
        odd_logits = self.odd_expert(features)

        super_pred = super_logits.argmax(dim=1)

        even_tensor = torch.tensor([0, 2, 6, 4, 8]).to("cuda")
        odd_tensor = torch.tensor([1, 3, 5, 7, 9]).to("cuda")
        even_pred = even_tensor[even_logits.argmax(dim=1)]
        odd_pred = odd_tensor[odd_logits.argmax(dim=1)]

        digit_pred = torch.where(super_pred == 0, even_pred, odd_pred)

        return super_logits, even_logits, odd_logits, digit_pred
    
#########################################################################
#def decode_prediction(super_logits, even_logits, odd_logits, device):  #
#                                                                       #
#   super_pred = super_logits.argmax(dim=1)                             #
#                                                                       #
#    even_tensor = torch.tensor([0, 2, 6, 4, 8]).to(device=device)      #
#    odd_tensor = torch.tensor([1, 3, 5, 7, 9]).to(device=device)       #
#    even_pred = even_tensor[even_logits.argmax(dim=1)]                 #
#    odd_pred = odd_tensor[odd_logits.argmax(dim=1)]                    #
#                                                                       # 
#    digit_pred = torch.where(super_pred == 0, even_pred, odd_pred)     #
...
def compute_loss(criterion, super_label, super_pred, digit_label, even_logits, odd_logits, device):

    loss_super = criterion(super_pred, super_label)

    even_mask = (super_label == 0)
    odd_mask = (super_label == 1)

    even_local = torch.tensor([[0,2,4,6,8].index(d.item()) for d in digit_label[even_mask]], dtype=torch.long).to(device=device)
    odd_local = torch.tensor([[1,3,5,7,9].index(d.item()) for d in digit_label[odd_mask]], dtype=torch.long).to(device=device)

    loss_even = criterion(even_logits[even_mask], even_local) if even_mask.any() else 0
    loss_odd = criterion(odd_logits[odd_mask], odd_local) if odd_mask.any() else 0

    return loss_super + loss_even + loss_odd
...
def train(model, data, criterion, optimizer, device, size):
    ...
    for img, super, digit in data:
        ...
        super_pred, even, odd, digit_pred = model(img)

        loss = compute_loss(criterion, super, super_pred, digit, even, odd, device)
        ...
        correct_digit += digit_pred.eq(digit).sum().item()
...
full_size = MNIST(root='/home/eriseldo/Desktop/Bachelorarbeit', train=True, transform=train_transformer)
